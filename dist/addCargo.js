...
/* document.addEventListener('DOMContentLoaded', () => {
    ...
    editCargoForm.addEventListener('submit', (e) => {
        ...
        const rowIndex = allItems.findIndex(row => row.querySelector('td').textContent === cargoId);
        if (rowIndex !== -1) {
            allItems[rowIndex].innerHTML = `
                <td class="py-2 px-4">${cargoId}</td>
                <td class="py-2 px-4">${type}</td>
                <td class="py-2 px-4">${dateDepart}</td>
                <td class="py-2 px-4">${dateArrivee}</td>
                <td class="py-2 px-4">${distance}</td>
    ...
    function createNewCargoRow(id, type, dateDepart, dateArrivee, distance) {
        ...
        newRow.querySelector('.editCargoBtn').addEventListener('click', () => {
            ...
            document.getElementById('editDateDepart').value = cells[2].textContent;
            document.getElementById('editDateArrivee').value = cells[3].textContent;
            document.getElementById('editDistance').value = cells[4].textContent;
...
/* document.addEventListener('DOMContentLoaded', () => {
    ...
    addCargoForm.addEventListener('submit', (e) => {
        ...
        // Mettre à jour les éléments et la pagination sans réinitialiser la page
        updateItems(allItems.concat([newRow]));
    });
...
function setupPagination(items, wrapper, rowsPerPage) {
    wrapper.innerHTML = '';
    const pageCount = Math.ceil(items.length / rowsPerPage); // Calcul du nombre total de pages
...
function createPaginationButton(page, items) {
    ...
    button.addEventListener('click', () => {
        const start = (page - 1) * itemsPerPage;
        const end = start + itemsPerPage;
        renderPage(items.slice(start, end), page);
    });
    return button;
}
function updateItems(items) {
    allItems = items; // Keep an unfiltered copy of the items
    renderPage(items, currentPage); // Display the current page with updated items
}
...
function filterItems() {
    ...
    renderPage(filteredItems, 1); // Display the first page of filtered items
    renderPagination(filteredItems); // Update pagination based on filtered items
}
